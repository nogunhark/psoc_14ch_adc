...
{
    CY_SET_REG16(ADC_IRQ_INTC_VECTOR, (uint16) address);
}
...
{
    return (cyisraddress) CY_GET_REG16(ADC_IRQ_INTC_VECTOR);
}
...
uint8 ADC_IRQ_GetState(void) 
{
    ...
    return ((*ADC_IRQ_INTC_SET_EN & (uint8)ADC_IRQ__INTC_MASK) != 0u) ? 1u:0u;
}
